...
export default function Chat() {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (otherUser?.fcmToken && otherUser?.notificationsEnabled !== false) {
        sendPushNotification(
          otherUser.fcmToken,
          userData?.fullName || 'New Message',
          text,
          { type: 'chat', chatId }
        );
      }
      ...
      if (selectedMessage.senderId !== currentUser.uid && otherUser?.notificationsEnabled !== false) {
        ...
        if (otherUser?.fcmToken) {
          sendPushNotification(
            otherUser.fcmToken,
            'New Reaction',
            `${userData?.fullName || 'Someone'} reacted to your message: ${emoji}`,
            { type: 'reaction', chatId }
          );
        }
      }
    }
