...
import { ArrowDown, ArrowRight, Check, Clock3, Heart, Instagram, MapPin, Menu, MessageCircle, Phone, Send, Star, X } from 'lucide-react';
...
function Home() {
  ...
  return (
    <main className="min-h-screen">
      <header className="absolute left-0 right-0 top-0 z-40 text-[#f9f1e7]">
        <div className="shell flex h-[84px] items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={() => setMenuOpen((current) => !current)} aria-label={menuOpen ? 'Close menu' : 'Open menu'} className="flex h-11 w-11 items-center justify-center rounded-full border border-[#f9f1e7]/40 md:hidden">
              {menuOpen ? <X size={19} /> : <Menu size={19} />}
            </button>
          </div>
        </div>
        {menuOpen && (
          <div className="mx-4 rounded-2xl border border-[#f9f1e7]/20 bg-[#183331] p-5 shadow-2xl md:hidden">
          ...
          <div className="pointer-events-none relative z-0 mt-14 h-[320px] w-[320px] self-end rounded-full md:absolute md:bottom-[10%] md:right-[-4%] md:mt-0 md:h-[640px] md:w-[640px]">
            <div className="hero-art h-full w-full rounded-full border border-[#edab77]/35 p-3">
              <div className="image-wrap h-full w-full rounded-full border-[12px] border-[#edab77]/20">
                <img src="https://images.pexels.com/photos/291528/pexels-photo-291528.jpeg?auto=compress&cs=tinysrgb&w=1600" alt="A richly iced chocolate cake on a dessert table" className="h-full w-full object-cover" />
              </div>
            </div>
          </div>
        ...
        <div className="shell">
          ...
          <div className="grid gap-5 md:grid-cols-[1.14fr_.86fr]">
            <article className="group relative overflow-hidden rounded-[1.25rem] bg-[#bd603f] md:row-span-2 reveal">
              <div className="image-wrap h-[430px] md:h-[635px]"><img src={collections[0].image} alt="A chocolate celebration cake topped with strawberries" className="h-full w-full object-cover mix-blend-multiply opacity-90 transition-transform duration-700 group-hover:scale-105" /></div>
            ...
            {collections.slice(1).map((item, index) => (
              <article key={item.name} className={`group grid overflow-hidden rounded-[1.25rem] ${item.tone === 'sage' ? 'bg-[#c4d0c1]' : 'bg-[#e8bc7c]'} sm:grid-cols-[.85fr_1.15fr] reveal reveal-delay-${index + 1}`}>
                <div className="image-wrap min-h-[230px] sm:min-h-[260px]"><img src={item.image} alt={item.name} className="h-full w-full object-cover transition-transform duration-700 group-hover:scale-105" /></div>
      ...
      <section id="story" className="section-pad overflow-hidden bg-[#f9f1e7]">
        <div className="shell grid items-center gap-12 md:grid-cols-[.8fr_1.2fr] md:gap-24">
          <div className="relative mx-auto w-full max-w-[390px] reveal">
            ...
            <div className="image-wrap relative aspect-[4/5]"><img src="https://images.pexels.com/photos/3992382/pexels-photo-3992382.jpeg?auto=compress&cs=tinysrgb&w=900" alt="Hands carefully preparing a homemade cake" className="h-full w-full object-cover" /></div>
      ...
      <section id="reviews" className="section-pad bg-[#e7d8c8]">
        <div className="shell">
          ...
          <div className="grid items-stretch gap-5 md:grid-cols-[1fr_1.35fr]">
            <div className="relative min-h-[310px] overflow-hidden rounded-[1.25rem] bg-[#bd603f] p-8 text-[#f9f1e7] md:p-10 reveal"><div className="absolute -right-6 -top-8 font-serif text-[210px] leading-none text-[#e8bc7c]/30">“</div><div className="relative flex h-full flex-col justify-between"><div className="flex gap-1 text-[#f3cb8c]">{[1, 2, 3, 4, 5].map((star) => <Star key={star} size={16} fill="currentColor" />)}</div><div><p className="display max-w-[420px] text-3xl leading-[1.08]">“{reviews[activeReview].quote}”</p><p className="mt-6 text-xs font-bold uppercase tracking-[.12em] text-[#f9f1e7]/70">{reviews[activeReview].name}</p></div></div></div>
